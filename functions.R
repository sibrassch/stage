...
# negative loglikelihood for switch model ============================================================================================================
switchNLL <- function(params, choice, reward,
                      threshold = 3,
                      V_A_init = c(green = 0.5, blue = 0.5),
                      V_B_init = c(green = 0.5, blue = 0.5),
                      active_state_init = "A") {
  ...
  for (t in 1:n) {
    ...
    if (active_state == "A") {
      V_A <- value_update(V_A, choice[t], reward[t], alpha)
    } else {
      V_B <- value_update(V_B, choice[t], reward[t], alpha)
    }
